...
```python
a = Director.objects.get(name="봉준호")

print(f"id : {a.id}")
print(f"name : {a.name}")
print(f"debut : {a.debut}")
...
for movi in movies:
	director_ = Director.objects.get(name=movi[0])
	genre_ = Genre.objects.get(title=movi[1])
    
    movie = Movie()
	movie.director = director_
	movie.genre = genre_
	movie.title = movi[2]
	movie.opening_date = movi[3]
	movie.running_time = movi[4]
	movie.screening = movi[5]
	movie.save()
...
for elem in a:
    print(elem.director, elem.genre, elem.title,\
          elem.opening_date, elem.running_time, elem.screening)
...
for elem in a:
    print(elem.director.name, elem.genre, elem.title,\
          elem.opening_date, elem.running_time, elem.screening)
```
...
for elem in a:
    print(elem.director.name, elem.genre.title, elem.title,\
          elem.opening_date, elem.running_time, elem.screening)
```
...
```python
a = Movie.objects.order_by("-opening_date")
...
```python
elem = Movie.objects.order_by("opening_date")[0]
...
```python
movies = Movie.objects.order_by("opening_date").filter(screening=True)
...
movies = Movie.objects.order_by("opening_date").filter(director=Bong)
...
```python
movies = Movie.objects.order_by("running_time").filter(running_time__gt=119)
...
```python
movies = Movie.objects.order_by("running_time").filter(running_time__gte=119)
...
movies = Movie.objects.order_by("opening_date")\
    .filter(opening_date__range=(start_date,end_date))
...
movies = Movie.objects.order_by("opening_date")\
    .filter(director=Bong, genre=drama)
